...
PKG_SUBDIRS=$(shell go list ./... | grep -v /vendor/ | sed -r 's|github.com/elixirhealth/timeline/||g' | sort)
...
acceptance:
	...
	@mkdir -p artifacts
	@go test -tags acceptance -v github.com/elixirhealth/timeline/pkg/acceptance 2>&1 | tee artifacts/acceptance.log
...
build-static:
	...
	@./vendor/$(SERVICE_BASE_PKG)/scripts/build-static deploy/bin/timeline
...
proto:
	@echo "--> Running protoc"
	@protoc pkg/timelineapi/timeline.proto -I. -I vendor/ --go_out=plugins=grpc:.
